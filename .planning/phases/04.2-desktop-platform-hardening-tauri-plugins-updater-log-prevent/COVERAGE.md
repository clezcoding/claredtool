...
|------------|---------|-------------|--------|
| Check version (Tauri dynamic JSON) | `GET /checkVersion?…&updater=tauri&channel=stable` | **INTEGRATE** | Core DESK-04 / D-05/D-07/D-11 |
...
| Performance tracing | tracesSampleRate | **OPT-OUT** (or ≤0.1 staging only) | D-39 off by default in 4.2 |
...
| `tauri-plugin-sentry-api` npm | companion package | **OPT-OUT unless verified** | SUS — prefer `@sentry/browser` + crates plugin (Plan 04 gate) |
